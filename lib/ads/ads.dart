...
/// APPLOVIN ADS STEP: 4 [CREATE CLASS]
 ...
 */
  ...
  static RxBool isBanner = false.obs;
  static RxBool isInterstital = false.obs;
  static RxBool isRewarded = false.obs;
  static RxBool isNative = false.obs;

  static double mediaViewAspectRatio = 16 / 9;
  static final FirebaseFirestore fireStore = FirebaseFirestore.instance;

  /// GET APP DETAILS FROM FIREBASE FIRESTORE
  static Future<void> appDetails() async {
    final DocumentSnapshot<Map<String, dynamic>> collection = await fireStore.collection('AppDetails').doc('Ads').get();

    if (collection.exists) {
      final Map<String, dynamic> data = collection.data() as Map<String, dynamic>;
      showAds.value = (isSubscribe.value ? false : data['showAds']);
      if (showAds.value) {
        _sdkKey = data['ApplovinKey'];
        isBanner.value = data['showBanner'];
        isInterstital.value = data['showInterstitial'];
        isRewarded.value = data['showRewarded'];
        isNative.value = data['showNative'];
        adLimit.value = (data['Limit'] ?? 3) as int;
        if (isBanner.value) {
          bannerAdUnitId = Platform.isIOS ? data['Banner']['IOS'] : data['Banner']['Android'];
        }
        if (isInterstital.value) {
          interstitialAdUnitId = Platform.isIOS ? data['Interstitial']['IOS'] : data['Interstitial']['Android'];
        }
        if (isRewarded.value) {
          rewardedAdUnitId = Platform.isIOS ? data['Rewarded']['IOS'] : data['Rewarded']['Android'];
        }
        if (isNative.value) {
          nativeAdUnitId = Platform.isIOS ? data['Native']['IOS'] : data['Native']['Android'];
        }
      }
    }
  }
  ...
  static Widget showBannerAds() {
    return Obx(() {
      isInitialized.value;
      if (isInitialized.value && isBanner.value && !isSubscribe.value) {
        return MaxAdView(
  ...
  static Future<bool> showInterAds() async {
    if (isInitialized.value && isInterstital.value && !isSubscribe.value) {
  ...
  static Future<bool> showRewardAds() async {
    if (isInitialized.value && isRewarded.value && !isSubscribe.value) {
  ...
  static Widget showNativeAds(context, double height) {
    return Obx(() {
      isInitialized.value;
      if (isInitialized.value && isNative.value && !isSubscribe.value) {
        return Container(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: MaxNativeAdCallToActionView(
                      style: ButtonStyle(
                        backgroundColor: WidgetStatePropertyAll<Color>(Theme.of(context).hintColor),
                        textStyle: WidgetStatePropertyAll<TextStyle>(TextStyle(fontSize: 15, fontFamily: "B")),
                      ),
                    ),
                  ),
                ],
  ...
  static void attachAdListeners() {
    ...
    AppLovinMAX.setRewardedAdListener(RewardedAdListener(
      ...
      onAdHiddenCallback: (ad) {
        final ind = Storages.read('index');
        if (ind != null) {
          late CreateController cc;
          if (Get.isRegistered<CreateController>()) {
            cc = Get.find<CreateController>();
          } else {
            cc = Get.put(CreateController());
          }
          cc.selectedFontFamilyIndex.value = ind as int;
        }
